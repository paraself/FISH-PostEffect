...
//		d. - then blend with the not glow plant - glowrt [pass 3]
	public static void Compose ( RenderTexture source, RenderTexture destination, bool isGlowOn, GlowType glowType, RenderTexture glowRT, RenderTexture glowBlurRT, bool isColorOn, Color multiplyColor,Material composeMtl) {
		if (composeMtl == null) {
	...
	public static void UnityBlurGlow ( RenderTexture source , RenderTexture dest , bool isGlowOn , RenderTexture glowRT , BlurType glowBlurType, int downsample, float blurSize, int blurIterations, Material blurMtl,  bool isColorOn, Color multiplyColor , Material composeMtl) {
		...
		Compose(source,dest,isGlowOn,GlowType.UnityBlur, glowRT,glowBlurredRT,isColorOn,multiplyColor,composeMtl);
	...
	public static void HDBlurGlow ( RenderTexture source, RenderTexture dest, bool isGlowOn, RenderTexture glowRT, HDBlurSettings hdBlurSetting, bool isColorOn, Color multiplyColor,Material composeMtl) {
		...
		Compose(source,dest,isGlowOn,GlowType.HDBlur,glowRT,glowBlurredRT,isColorOn,multiplyColor,composeMtl);
		if (isGlowOn) RenderTexture.ReleaseTemporary(glowBlurredRT);
	}
